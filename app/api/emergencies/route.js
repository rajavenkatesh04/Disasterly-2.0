...
export async function GET(request) {
    try {
        ...
        try {
            ...
            let emergencies;
            if (all) {
                emergencies = await collection.find().toArray();
            } else if (userId) {
                emergencies = await collection.find({ raisedBy: userId }).toArray();
            } else {
...
export async function PATCH(request) {
    try {
        ...
        try {
            ...
            return NextResponse.json({ success: true, message: 'Emergency updated successfully' });
        } finally {
            await client.close();
        }
    } catch (error) {
        ...
        return NextResponse.json(
            { success: false, message: 'Failed to update emergency' },
            { status: 500 }
        );
    }
}
